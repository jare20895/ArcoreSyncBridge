...
export default function EditDatabase() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        application_id: dbData.application_id || '',
        name: dbData.name || '',
        db_type: dbData.db_type || 'POSTGRES',
        environment: dbData.environment || 'DEV',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateDatabase(id as string, formData);
      router.push('/databases');
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to update database');
    }
  };
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary">Edit Database</h1>
        <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-1">Update database details</p>
      </div>
