...
- [x] Ensure sync_cursors UUID defaults work in Postgres (pgcrypto or app-side UUIDs)
...
- [x] Use SharePointConnection.client_secret as primary auth source (env fallback for local dev)
...
- [x] Use DatabaseInstance credentials in DB client/introspection/connection tests
- [x] Expose DatabaseInstance credentials in API schemas/UI
- [x] Persist source schema/table name and field mapping names via API
- [x] Use cursor_column_id for UPDATED_AT queries
- [x] Persist field mappings and sync targets during provisioning
...
- [x] Store SharePoint connection_id + site_id on SyncTarget (per-definition context)
- [x] Scope SyncLedgerEntry by sync_def_id (unique per definition)
- [x] Apply sharding automatically for target_strategy=CONDITIONAL; allow per-run override for single target
- [x] Remove env-based SharePoint context from moves/drift
...
- [x] Update CursorService to use SyncCursor surrogate ID + scoped indexes
